...
class SongDetail extends React.Component {
    ...
    componentDidMount() {
        ...
        const headerImg = (query && query.img) ? query.img : search.split('&')[2].split('=')[1];
        this.setState({
            img:headerImg
        })
    ...
    getSongMsg(songId, type) {
        store.dispatch(songListAddAction({
    ...
    render() {
        ...
        return (
            <section className="song-detail">
                 <h4>我播放页面</h4>
                <div className="song-bg" style={{ backgroundImage: `url(${this.state.img})` }}></div>
                                ...
                                <Col className="banner-elem">
                                    <TweenOne className="banner-content" animation={{ x: -100, opacity: 0, type: 'from', delay: 100 }}>
                                        <Row type={'flex'} gutter={10} align='middle' justify={'space-between'} style={{ flexDirection: 'column', paddingBottom: '25px' }}>
                                            <Col style={{ fontSize: '24px', color: '#f3f3f3', padding: '0 20px' }}>{this.props.songPlayCur.lever}</Col>
                                    ...
                                    <TweenOne className="banner-content" animation={{ x: -100, opacity: 0, type: 'from', delay: 300 }}>
                                        <Col className={`${this.props.songPlayStatus ? 'playing' : 'paused'} song-pic`} style={{ backgroundImage: `url(${this.state.img})`, borderRadius: '50%', margin: '0 auto', width: '256px', height: '256px', backgroundSize: 'cover' }}></Col>
                                    </TweenOne>
                                </Col>
                                :
                        ...
                        <Row type={'flex'} align='middle' justify={'space-between'}>
                            {
                                this.props.songPlayMode == 0 ?
                                    ...
                                    :
                                    <Avatar className="play-mode" src={this.props.songPlayCur.url} style={{ fontSize: '30px', color: '#f3f3f3' }} size={30} onClick={this.changePlayMode}></Avatar>
                            }
